...
class CharList extends Component {
  ...
  createList = (charList) => {
    const charItem = charList.map(item => {
      ...
      if(item.imgSrc == 'http://i.annihil.us/u/prod/marvel/i/mg/b/40/image_not_available.jpg'){
        styleImg = {'objectFit':'fill'}
      }
      return ( <li className="char__item" key={item.id} onClick={()=>{this.props.onSelectedIdChar(item.id)}}>
  ...
  render(){
    ...
    return (
        <div className="char__list">
            {content}{errorMessage}{spinnerMessage}
